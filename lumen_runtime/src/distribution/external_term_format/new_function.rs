...
pub fn decode<'a>(
    ...
    let (total_byte_len, after_size_bytes) = u32::decode(process, bytes)?;
    let (arity, after_arity_bytes) = u8::decode(process, after_size_bytes)?;
    let (uniq, after_uniq_bytes) = decode_uniq(process, after_arity_bytes)?;
    let (index, after_index_bytes) = u32::decode(process, after_uniq_bytes)?;
    let (num_free, after_num_free_bytes) = u32::decode(process, after_index_bytes)?;
    let (module, after_module_bytes) = atom::decode_tagged(process, safe, after_num_free_bytes)?;

    let (old_index, after_old_index_bytes) = isize::decode(process, after_module_bytes)?;
    assert_eq!(old_index, index as isize);

    let (old_uniq, after_old_uniq_bytes) = isize::decode(process, after_old_index_bytes)?;
    let old_unique = old_uniq as OldUnique;

    let (creator, after_creator_bytes) = Pid::decode(process, safe, after_old_uniq_bytes)?;
...
fn decode_uniq<'a>(
    process: &Process,
    bytes: &'a [u8],
) -> Result<([u8; UNIQ_LEN], &'a [u8]), Exception> {
